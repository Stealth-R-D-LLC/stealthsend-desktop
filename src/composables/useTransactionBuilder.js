...
export default async function useTransactionBuilder(utxo, sendForm) {
  ...
  async function buildTransaction() {
    ...
    console.log(
      ...
      floor(calculateChange(sumUtxo, Number(sendForm.amount)) * 1e6), calculateChange(sumUtxo, Number(sendForm.amount))
    );
    ...
    if (calculateChange(sumUtxo, Number(sendForm.amount)) > CryptoService.constraints.MINIMAL_CHANGE) {
