...
class Psbt {
  ...
  addInput(inputData) {
    ...
    const txIn = c.__TX.ins[c.__TX.ins.length - 1]; // nosemgrep semgrep.js-frontend.security.audit.detect-bracket-object-injection
    checkTxInputCache(c, txIn);
    const inputIndex = this.data.inputs.length - 1;
    const input = this.data.inputs[inputIndex]; // nosemgrep semgrep.js-frontend.security.audit.detect-bracket-object-injection
...
function redeemFromFinalScriptSig(finalScript) {
  ...
  if (!decomp) return;
  const lastItem = decomp[decomp.length - 1]; // nosemgrep semgrep.js-frontend.security.audit.detect-bracket-object-injection
  if (
